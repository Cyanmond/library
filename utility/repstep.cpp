...
class repstep {
    struct repstep_iterator {
        usize itr, stepsize;
        constexpr repstep_iterator(const usize pos, const usize stp) noexcept : itr(pos), stepsize(stp) {}
  ...
  public:
    constexpr repstep(const usize first_, const usize last_, const usize stp_) : first(first_, stp_), last(last_) {}
